...
function getModelTypeProperties(node, genericTypes) {
    if (node.kind === ts.SyntaxKind.IntersectionType) {
        ...
        return intersectionTypeNode.types.reduce(function (properties, typeNode) {
    ...
    if (node.kind === ts.SyntaxKind.TypeReference) {
        ...
        var typeChecker = metadataGenerator_1.MetadataGenerator.current.typeChecker; // Assuming you have access to a type checker instance
        var symbol = typeChecker.getSymbolAtLocation(typeReferenceNode.typeName);
        if (symbol) {
            var declarations = symbol.getDeclarations();
            if (declarations && declarations.length > 0) {
                // Process the first declaration (could be an interface, class, or type alias)
                var resolvedDeclaration = declarations[0];
...
function hasPublicMemberModifier(node) {
    if (ts.canHaveModifiers(node)) {
        ...
        return !modifiers || modifiers.every(function (modifier) { return ![ts.SyntaxKind.ProtectedKeyword, ts.SyntaxKind.PrivateKeyword].includes(modifier.kind); });
    }
    return true;
}
function hasPublicConstructorModifier(node) {
    if (ts.canHaveModifiers(node)) {
        var modifiers = ts.getModifiers(node);
        return modifiers && modifiers.some(function (modifier) { return modifier.kind === ts.SyntaxKind.PublicKeyword; });
    }
    return false;
}
